...
export default function GoalFaceMap({ shots, title, activeLocation, onLocationClick }) {
  ...
  const sectionStats = LOCATIONS.map(sec => {
    ...
    const saves = secShots.filter(s =>
      s.shotOutcome === 'Canada Save' ||
      s.shotOutcome === 'Canada Ball Control Save' ||
      s.shotOutcome === 'Opponent Save'
    ).length
  ...
  return (
    <div className="card">
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H + 60}`} style={{ overflow: 'visible' }}>
          ...
          return (
            <g key={sec} style={{ cursor: onLocationClick ? 'pointer' : 'default' }}
              ...
              <rect x={x} y={MARGIN.top} width={SEC_W} height={GOAL_H}
                fill={isActive ? `rgba(245,158,11,${0.15 + intensity * 0.35})` : `rgba(30,58,138,${0.15 + intensity * 0.5})`}
        ...
        {/* Goal posts */}
        <line x1={MARGIN.left} y1={MARGIN.top} x2={MARGIN.left} y2={MARGIN.top + GOAL_H}
          stroke="#f59e0b" strokeWidth={3} />
        <line x1={W - MARGIN.right} y1={MARGIN.top} x2={W - MARGIN.right} y2={MARGIN.top + GOAL_H}
          stroke="#f59e0b" strokeWidth={3} />
        <line x1={MARGIN.left} y1={MARGIN.top} x2={W - MARGIN.right} y2={MARGIN.top}
          stroke="#f59e0b" strokeWidth={3} />
        <line x1={MARGIN.left} y1={MARGIN.top + GOAL_H} x2={W - MARGIN.right} y2={MARGIN.top + GOAL_H}
        ...
        {goalsInGoal.map((shot, i) => {
          ...
          const color = getOutcomeColor(shot.shotOutcome)
          return (
            <circle
              key={i} cx={x} cy={y} r={5}
        ...
        {outLeft.slice(0, 6).map((shot, i) => {
          ...
          return (
            <circle key={`outl-${i}`} cx={15} cy={MARGIN.top + 15 + i * 14} r={4}
              fill={color} fillOpacity={0.7} stroke="#000" strokeWidth={0.5} />
          )
        })}
        {outLeft.length > 6 && (
        ...
        <text x={W / 2} y={MARGIN.top + GOAL_H + 50}
          ...
          Goal Sections (1 = Left Post → 5 = Right Post) · Click a section to filter all data
        </text>
      </svg>

      {/* Legend */}
      <div className="legend">
        <LegendDot color="#22c55e" label="Canada Goal" />
        <LegendDot color="#ef4444" label="Goal Conceded" />
        <LegendDot color="#3b82f6" label="Canada Save" />
        <LegendDot color="#f59e0b" label="Opp Save" />
      ...
      {tooltip && (
        <div className="tooltip" style={{ left: tooltip.x + 10, top: tooltip.y - 30 }}>
          ...
          <div className="tooltip-sub">{tooltip.shot.opponent} ({tooltip.shot.tournament})</div>
        </div>
      )}
    </div>
  )
